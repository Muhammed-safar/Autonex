...
export const createOrder = async (checkoutId, paymentDetails) => {
  ...
  try {
    ...
    // Create Order
    const orderStatus = isCOD ? "PLACED" : "CONFIRMED";
...
Your order status has been updated to ${status.replace(/_/g, " ")}.`;
    ...
    sendWhatsAppMessage(updatedOrder.shippingAddress.phone, message).catch(
      (error) => {
        console.error(
          `Failed to send WhatsApp notification for order ${updatedOrder.orderNumber}:`,
          error,
        );
      },
    );
...
export const cancelOrder = async ({ orderId, user, ipAddress, userAgent }) => {
  if (!mongoose.Types.ObjectId.isValid(orderId)) {
    throw new Error("Invalid Order ID");
  }
  ...
  try {
    ...
    const cancelledOrder = await Order.findById(order._id).populate(
      "user",
      "name email",
    );
...
export const getOrderInvoice = async (orderId, user) => {
  ...
  // Permission
  const isOwner = order.user._id.toString() === user.id.toString();
